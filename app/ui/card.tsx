...
export default function Card({ recipes }: { recipes: FetchRecipes }) {
  const { openViewModal, user } = useBoundStore();
          ...
          <div className="w-full mt-3 relative px-1">
            ...
            <div className="flex justify-between items-end px-1">
              ...
              {recipes.tags.map((tag, index) =>(
                <p key={index} className="font-sans text-sm bg-gradient-to-r from-blue-600 to-cyan-600 bg-clip-text text-transparent cursor-pointer mt-1 w-[70%] truncate">
                #{tag}
              </p>
              ) )}
              </div>
